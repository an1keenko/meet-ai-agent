...
export const meetingssRouter = createTRPCRouter({
  ...
  connectAgent: protectedProcedure.input(z.object({ id: z.string() })).mutation(async ({ input }) => {
    const [existingMeeting] = await db.select().from(meetings).where(eq(meetings.id, input.id));
